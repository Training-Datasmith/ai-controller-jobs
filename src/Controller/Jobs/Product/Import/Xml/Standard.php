...
class Standard extends \Aimeos\Controller\Jobs\Base implements \Aimeos\Controller\Jobs\Iface
{
    ...
    public function run(): void
    {
        ...
        try {
            ...
            $logger->error('Product import error: ' . $e->get_message() . "\n" . $e->get_trace_as_string(), 'import/xml/product');
            $this->mail('Product XML import error', $e->get_message());
            throw $e;
        }
    }
    /**
    ...
    protected function import(\Aimeos\M_Shop\Context_Iface $context, string $path)
    {
        ...
        while ($xml->read() === true) {
            if ($xml->depth === 1 && $xml->node_type === \Xml_Reader::ELEMENT && $xml->name === 'productitem') {
    ...
    protected function import_nodes(array $nodes)
    {
        ...
        foreach ($nodes as $node) {
            $ref = $node->attributes->get_named_item('ref')?->node_value;
            $item = $map->get($ref) ?: $manager->create();
            if ($ref && $replace) {
                $item->set_id($ref);
            }
            $manager->save($this->process($item, $node));
            $this->add_type('product/type', 'product', $item->get_type());
        }
    }
    /**
     ...
     */
    protected function process(\Aimeos\M_Shop\Product\Item\Iface $item, \Dom_Element $node): \Aimeos\M_Shop\Product\Item\Iface
    {
        try {
            $list = [];
            ...
            $msg = 'Product import error: ' . $e->get_message() . "\n" . $e->get_trace_as_string();
    ...
    protected function products(array $nodes): \Aimeos\Map
    {
        $codes = [];
        foreach ($nodes as $node) {
            if ($attr = $node->attributes->get_named_item('ref')) {
