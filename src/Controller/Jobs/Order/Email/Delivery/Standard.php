...
class Standard extends \Aimeos\Controller\Jobs\Base implements \Aimeos\Controller\Jobs\Iface
{
    ...
    {
        if (($addr = current($basket->get_address('delivery'))) !== false && $addr->get_email()) {
            return $addr;
        }
    ...
    protected function filter(\Aimeos\Base\Criteria\Iface $filter, int $status): \Aimeos\Base\Criteria\Iface
    {
        ...
        $filter->add($filter->and([$filter->compare('>=', 'order.mtime', $limit_date), $filter->compare('==', 'order.statusdelivery', $status), $filter->compare('==', $filter->make('order:status', $param), 0)]));
        return $filter;
    }
    /**
    ...
    protected function notify(\Aimeos\Map $items, int $status)
    {
        ...
        foreach ($items as $id => $item) {
            try {
                ...
                $str = sprintf('Sent order delivery e-mail for order "%1$s" and status "%2$s"', $item->get_id(), $status);
                ...
                $context->logger()->error($msg . PHP_EOL . $e->get_trace_as_string(), 'email/order/delivery');
            }
        }
    }
    /**
    ...
    protected function send(\Aimeos\M_Shop\Order\Item\Iface $order, ?string $theme = null, ?string $logo_path = null)
    {
        ...
        $msg->subject(sprintf($context->translate('controller/jobs', 'Your order %1$s'), $order->get_invoice_number()))->html($view->render($config->get('controller/jobs/order/email/delivery/template-html', 'order/email/delivery/html')))->text($view->render($config->get('controller/jobs/order/email/delivery/template-text', 'order/email/delivery/text')))->send();
    }
    /**
    ...
    {
        $default = [\Aimeos\M_Shop\Order\Item\Base::STAT_PROGRESS, \Aimeos\M_Shop\Order\Item\Base::STAT_DISPATCHED, \Aimeos\M_Shop\Order\Item\Base::STAT_REFUSED, \Aimeos\M_Shop\Order\Item\Base::STAT_RETURNED];
    ...
    {
        return \Aimeos\M_Shop\Order\Item\Status\Base::EMAIL_DELIVERY;
    }
    /**
