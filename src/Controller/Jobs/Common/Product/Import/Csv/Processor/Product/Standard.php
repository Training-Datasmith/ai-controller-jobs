...
class Standard extends \Aimeos\Controller\Jobs\Common\Product\Import\Csv\Processor\Base implements \Aimeos\Controller\Jobs\Common\Product\Import\Csv\Processor\Iface
{
    ...
    public function __construct(\Aimeos\M_Shop\Context_Iface $context, array $mapping, ?\Aimeos\Controller\Jobs\Common\Product\Import\Csv\Processor\Iface $object = null)
    {
        ...
        $this->list_types = $config->get('controller/jobs/product/import/csv/product/listtypes', $default);
        ...
        $this->cache = $this->get_cache('product');
    }
    /**
    ...
    public function process(\Aimeos\M_Shop\Product\Item\Iface $product, array $data): array
    {
        ...
        $list_items = $product->get_list_items('product', null, null, false);
        ...
        foreach ($this->get_mapped_chunk($data, $this->get_mapping()) as $list) {
            ...
            $this->add_type('product/lists/type', 'product', $listtype);
            foreach (explode($separator, trim($this->val($list, 'product.code', ''))) as $code) {
                ...
                if (($list_item = $product->get_list_item('product', $listtype, $prodid)) === null) {
                    $list_item = $manager->create_list_item()->set_type($listtype);
                } else {
                    unset($list_items[$list_item->get_id()]);
                }
                $list_item = $list_item->from_array($list)->set_ref_id($prodid)->set_config($list_config)->set_position($pos++);
                $product->add_list_item('product', $list_item);
            }
        }
        $product->delete_list_items($list_items->to_array());
