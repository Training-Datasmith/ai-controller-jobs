...
class Standard extends \Aimeos\Controller\Jobs\Base implements \Aimeos\Controller\Jobs\Iface
{
    ...
    public function run(): void
    {
        ...
        try {
            ...
            $logger->error('Supplier import error: ' . $e->get_message() . "\n" . $e->get_trace_as_string(), 'import/xml/supplier');
            $this->mail('Supplier XML import error', $e->get_message());
            throw $e;
        }
    }
    /**
    ...
    protected function import(\Aimeos\M_Shop\Context_Iface $context, string $path)
    {
        ...
        while ($xml->read() === true) {
            if ($xml->depth === 1 && $xml->node_type === \Xml_Reader::ELEMENT && $xml->name === 'supplieritem') {
     ...
     */
    protected function process(\Aimeos\M_Shop\Supplier\Item\Iface $item, \Dom_Element $node): \Aimeos\M_Shop\Supplier\Item\Iface
    {
        try {
            $list = [];
            ...
            foreach ($node->child_nodes as $tag) {
                if (in_array($tag->node_name, ['address', 'lists'])) {
            ...
            $msg = 'Supplier import error: ' . $e->get_message() . "\n" . $e->get_trace_as_string();
